...
const MS_ID_DAY = 86400000;

const mapStateToProps = ({cards, showBack}, {params: {deckId}}) => ({
    showBack,
    deckId,
    card: cards.filter((card) => card.deckId === deckId &&
        (!card.lastStudiedOn || (new Date - card.lastStudiedOn) / MS_ID_DAY >= card.score))[0]
});
...
const StudyModal = ({card, showBack, onFlip, deckId, onStudied}) => {
    let body = (<div className="no-cards">
        <p>You have no cards to study in this deck right now. Hurray!</p>
    </div>)
    if (card) {
        body = (<div className="study-card">
            ...
            <div className={showBack ? "back" : "back hide"}>
                <div>
                    <p>{card.back ? card.back : "No back in this card"}</p>
                </div>
                <p>How did you do?</p>
                <p>
